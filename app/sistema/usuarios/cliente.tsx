...
const pendientesAprobacion = usuarios.filter((u) => !u.activo)
  ...
  return (
    <>
      ...
      {/* ── MENSAJE ── */}
      {mensaje && (
        <div style={{ background: T.greenAlpha, borderWidth: '0.5px', borderStyle: 'solid', borderColor: `${T.green}44`, color: T.green, padding: '10px 16px', borderRadius: 8, fontSize: 13, fontWeight: 500, marginBottom: 16 }}>
          {mensaje}
        </div>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 16, marginBottom: 24 }}>
        {[
          ...
          <div key={label} style={{ background: T.surface, borderWidth: '0.5px', borderStyle: 'solid', borderColor: T.border, borderRadius: 12, padding: '16px 20px' }}>
      ...
      <div className="u-filtros">
        <div className="u-search">
          ...
          <input
            ...
            style={{ width: '100%', padding: '9px 12px 9px 33px', background: T.surfaceLow, borderWidth: '0.5px', borderStyle: 'solid', borderColor: T.border, borderRadius: 8, color: T.textPrimary, fontSize: 13, outline: 'none' }}
          />
        </div>
        <select value={filtroRol} onChange={(e) => setFiltroRol(e.target.value)}
          style={{ padding: '9px 14px', background: T.surfaceLow, borderWidth: '0.5px', borderStyle: 'solid', borderColor: T.border, borderRadius: 8, color: T.textPrimary, fontSize: 13, cursor: 'pointer', outline: 'none' }}>
      ...
      {/* ── TABLA (ESCRITORIO) ── */}
      <div className="u-tabla-desktop" style={{ background: T.surface, borderWidth: '0.5px', borderStyle: 'solid', borderColor: T.border, borderRadius: 12, overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, textAlign: 'left' }}>
          <thead>
            <tr style={{ color: T.textMuted, borderBottomWidth: '0.5px', borderBottomStyle: 'solid', borderBottomColor: T.border, height: 40 }}>
          ...
          <tbody>
            {usuariosFiltrados.length === 0 ? (
              ...
              return (
                <tr key={usuario.id} style={{ borderBottomWidth: '0.5px', borderBottomStyle: 'solid', borderBottomColor: T.border }}>
                  <td style={{ padding: '12px 12px', display: 'flex', alignItems: 'center', gap: 12 }}>
                    <div style={{ width: 32, height: 32, borderRadius: '50%', background: T.surfaceLow, borderWidth: '0.5px', borderStyle: 'solid', borderColor: T.border, color: T.textMuted, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, flexShrink: 0 }}>
                  ...
                  <td>
                    {/* Se removió la propiedad abreviada 'border' para corregir el conflicto con 'borderColor' */}
                    <span style={{ borderWidth: '0.5px', borderStyle: 'solid', padding: '2px 8px', borderRadius: 6, fontSize: 11, fontWeight: 600, background: esAdmin ? T.accentAlpha : T.goldAlpha, color: esAdmin ? T.textAccent : T.gold, borderColor: esAdmin ? T.accent : T.gold }}>
                      {usuario.rol}
                    </span>
                  </td>
                  <td>
      ...
      <div className="u-tarjetas-mobile" style={{ flexDirection: 'column', gap: 12 }}>
        {usuariosFiltrados.length === 0 ? (
          <div style={{ background: T.surface, borderWidth: '0.5px', borderStyle: 'solid', borderColor: T.border, borderRadius: 12, padding: 24, textAlign: 'center', color: T.textFaint }}>
            No se encontraron usuarios con esos criterios.
          </div>
        ) : usuariosFiltrados.map((usuario) => {
          const esAdmin  = usuario.rol?.toLowerCase() === 'admin'
          ...
          return (
            <div key={usuario.id} style={{ background: T.surface, borderWidth: '0.5px', borderStyle: 'solid', borderColor: T.border, borderRadius: 12, padding: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
              {/* Cabecera de la tarjeta: avatar + nombre + rol */}
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{ width: 40, height: 40, borderRadius: '50%', background: T.surfaceLow, borderWidth: '0.5px', borderStyle: 'solid', borderColor: T.border, color: T.textMuted, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 700, flexShrink: 0 }}>
                  {usuario.nombre_completo.substring(0, 2).toUpperCase()}
                </div>
                ...
                {/* Se corrigió 'border' aquí también */}
                <span style={{ borderWidth: '0.5px', borderStyle: 'solid', padding: '2px 8px', borderRadius: 6, fontSize: 11, fontWeight: 600, background: esAdmin ? T.accentAlpha : T.goldAlpha, color: esAdmin ? T.textAccent : T.gold, borderColor: esAdmin ? T.accent : T.gold, flexShrink: 0 }}>
                  {usuario.rol}
                </span>
              </div>
...
function getStyles(T: typeof T_DARK) {
  return {
    btnPrimario: {
      ...
      borderStyle: 'none', borderRadius: 9, fontSize: 13, fontWeight: 600,
      cursor: 'pointer', whiteSpace: 'nowrap', flexShrink: 0,
    } as React.CSSProperties,
    alertaPendientes: {
      display: 'flex', alignItems: 'center', gap: 9,
      background: T.amberAlpha, borderWidth: '0.5px', borderStyle: 'solid', borderColor: `${T.amber}44`, color: T.amber,
      padding: '11px 16px', borderRadius: 10, fontSize: 13, marginBottom: 16,
    } as React.CSSProperties,
    btnVerPerfil: {
      background: 'transparent', borderWidth: '0.5px', borderStyle: 'solid', borderColor: T.border,
      borderRadius: 6, padding: '5px 12px', fontSize: 12, fontWeight: 500,
      color: T.textMuted, cursor: 'pointer',
    } as React.CSSProperties,
    btnActivar: {
      background: T.greenAlpha, borderWidth: '0.5px', borderStyle: 'solid', borderColor: `${T.green}44`,
      borderRadius: 6, padding: '5px 12px', fontSize: 12, fontWeight: 600,
      color: T.green, cursor: 'pointer',
    } as React.CSSProperties,
    btnSuspender: {
      background: T.redAlpha, borderWidth: '0.5px', borderStyle: 'solid', borderColor: `${T.red}44`,
      borderRadius: 6, padding: '5px 12px', fontSize: 12, fontWeight: 600,
